...
export function SiteHeader(props: HeaderQuery["header"]) {
  ...
  const handleSubmenu = (index: React.SetStateAction<number>) => {
    if (openIndex === index) {
      setOpenIndex(-1);
    } else {
      setOpenIndex(index);
  ...
  return (
    <>
      ...
      <div className="hidden lg:flex lg:gap-x-12">
        <span data-tina-field={tinaField(props, "emailUs")}> emailUs: {props?.emailUs}</span>
        <span data-tina-field={tinaField(props, "callUsNumber")}
        > callUsNumber: {props?.callUsNumber}</span>
        <span data-tina-field={tinaField(props, "openingTime")}
        >openingTime: {props?.openingTime}</span>
      </div>
    </>
  );
}
